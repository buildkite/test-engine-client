...
func UploadCLI(flag *flag.FlagSet, env env.Env) error {
	...
	respData, err := Upload(cfg, runEnv, format, filename)
	if err != nil {
		return err
	}
	...
	slog.Info("Upload successful", logArgs...)
...
func Upload(cfg Config, runEnv runEnvMap, format string, filename string) (map[string]string, error) {
	...
	req, err := http.NewRequest(http.MethodPost, cfg.UploadUrl, body.buf)
	if err != nil {
...
func buildRunEnv(env env.Env) (runEnvMap, error) {
	runEnv := runEnvMap{
		"collector": "bktec",
