...
// UploadTestResults marshals results to a temp JSON file and POSTs it to the
// Test Engine analytics API at <c.ServerBaseURL>/v1/uploads.
// Errors are returned to the caller to be logged and suppressed — this upload
// is best-effort and must not fail the build.
func (c *Client) UploadTestResults(ctx context.Context, token string, results []runner.TestEngineTest) error {
	data, err := json.Marshal(results)
	if err != nil {
		return fmt.Errorf("marshaling test results: %w", err)
	}

	tmpFile, err := os.CreateTemp("", "te-test-results-*.json")
	if err != nil {
	...
	body, contentType, err := buildTestResultsMultipartBody(tmpFile.Name())
	if err != nil {
		return err
	}
...
func buildTestResultsMultipartBody(filePath string) (*bytes.Buffer, string, error) {
