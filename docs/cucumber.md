...
Cucumber uses [selector-based test splitting](../README.md#selector-based-test-splitting) by default. Each feature file path discovered with Cucumber's default `features/**/*.feature` pattern becomes a selector. You can customize which files are discovered with `BUILDKITE_TEST_ENGINE_TEST_FILE_PATTERN` and `BUILDKITE_TEST_ENGINE_TEST_FILE_EXCLUDE_PATTERN`; the `{{testExamples}}` command placeholder works as before.

> [!NOTE]
> If you upload results with the [Ruby test collector](https://buildkite.com/docs/test-engine/test-collection/ruby-collectors), we recommend updating to `buildkite-test_collector` v2.14.0 or later so selectors are attributed directly. Older versions can still upload results, and Test Engine [attempts to match selectors using the file path](../README.md#how-selectors-are-matched). If no history matches, Test Engine uses default duration estimates and bktec prints a warning. Updating matters most if you use a location prefix (`--location-prefix` / `BUILDKITE_TEST_ENGINE_LOCATION_PREFIX`), since the fallback only handles the prefix on a best-effort basis.
