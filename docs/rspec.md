...
RSpec uses [selector-based test splitting](../README.md#selector-based-test-splitting) by default. Each test file path discovered with RSpec's default `spec/**/*_spec.rb` pattern becomes a selector. You can customize which files are discovered with `BUILDKITE_TEST_ENGINE_TEST_FILE_PATTERN` and `BUILDKITE_TEST_ENGINE_TEST_FILE_EXCLUDE_PATTERN`; the `{{testExamples}}` command placeholder works as before.
