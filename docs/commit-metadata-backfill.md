...
bktec can collect historical git commit metadata from your repository and upload it to Buildkite. This data is used by the test prediction training pipeline to build models that power test selection, helping you run only the tests relevant to your code changes.

The backfill commands are available under `bktec tools` and are hidden from `bktec --help` unless the `BKTEC_PREVIEW_SELECTION` environment variable is enabled. The commands can always be invoked directly regardless of this setting.
...
- `BKTEC_PREVIEW_SELECTION` set to a truthy value (`1`, `true`, `yes`, or `on`) to see the commands in help output
...
**Skip full diffs to reduce upload size:**
```sh
