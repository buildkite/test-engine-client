...
| `BUILDKITE_SPLITTER_RETRY_COUNT` | `0` | The number of retries performed. Test splitter runs the test command defined in `BUILDKITE_SPLITTER_TEST_CMD`, and retries only the failed tests for a maximum of `BUILDKITE_SPLITTER_RETRY_COUNT` times with the retry command `BUILDKITE_SPLITTER_RETRY_CMD`. |
