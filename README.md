...
| `BUILDKITE_SPLITTER_RESULT_PATH` | - | The path to save the test result generated by the test runner. If the path includes `*`, it will be replaced by a UNIX timestamp. Test Splitter Client will append additional options to the test command to generate the output. For example, in RSpec, it will append `--format json --out $BUILDKITE_SPLITTER_RESULT_PATH` to the configured command. The output will be parsed at the end of each test execution to detect failed tests for retry purposes. If this option is not set, the Test Splitter client will store the result in a temporary file that will be deleted at the end of execution. |
