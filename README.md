...
| `BUILDKITE_SPLITTER_RETRY_CMD` | `BUILDKITE_SPLITTER_TEST_CMD` with `--only-failures` | The command to rerun the failed tests. For Rspec, it depends on `example_status_persistence_file_path` to be defined as part of your Rspec configuration for the retry to work. Please see [Rspec documentation](https://rspec.info/features/3-12/rspec-core/command-line/only-failures/) for available retry options.
| `BUILDKITE_SPLITTER_RETRY_COUNT` | `0` | The number of retries permitted. Test splitter runs the test command defined in `BUILDKITE_SPLITTER_TEST_CMD`, and retries only the failed tests for a maximum of `BUILDKITE_SPLITTER_RETRY_COUNT` times with the retry command `BUILDKITE_SPLITTER_RETRY_CMD`. |
